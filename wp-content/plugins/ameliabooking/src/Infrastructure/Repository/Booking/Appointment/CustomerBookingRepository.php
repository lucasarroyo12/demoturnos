...
class CustomerBookingRepository extends AbstractRepository implements CustomerBookingRepositoryInterface
{
     ...
     * @throws QueryExecutionException
     */
    public function updateStatusByPackageCustomerId($id, $status)
    {
        $packageCustomerServiceTable = PackagesCustomersServicesTable::getTableName();
        $params = [
            ':status' => $status,
            ':packageCustomerId' => $id
        ];

        try {
            $statement = $this->connection->prepare(
                "UPDATE {$this->table} cb
                INNER JOIN {$packageCustomerServiceTable} pcs ON pcs.id = cb.packageCustomerServiceId
                SET `status` = :status
